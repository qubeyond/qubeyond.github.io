...
> В качестве одно из путей может быть удаленная тачка в следующем формате `<user>@<ip>:<path to file>`. Пользуйтесь:3
