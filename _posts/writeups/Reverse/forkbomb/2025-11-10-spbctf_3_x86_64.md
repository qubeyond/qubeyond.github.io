...
Ключ, передаваеймый через агрументы командной строки, кодируется бинарным файлом. Для решения нужно найти исходую строку, для вот этого вывода:
...
Хмм. Есть подозрение, что функция `swapper` меняет буквы местами. При это результат сохраняется в `str`. С учетом того, что второй аргумент меняется линейно от 10 до 2, то у меня возникла идея - пропатчу я второй аргумент так, чтобы значения шли от 2 до 10:
