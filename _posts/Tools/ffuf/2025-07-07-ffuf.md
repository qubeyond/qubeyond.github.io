...
| `-recursion` | Рекурсивный скан. `URL` должен заканчиваться на `FUZZ` (по умолчанию: `false`) | 
...
| `-u` | Целевой `URL` с ключевым словом `FUZZ` | 
...
| `-w` | Путь к файлу `wordlist` и (необязательно) ключевое слово, разделенное двоеточием. Например, `/путь/к/wordlist:КЛЮЧЕВОЕ_СЛОВО` (по умолчанию: `FUZZ`) |
