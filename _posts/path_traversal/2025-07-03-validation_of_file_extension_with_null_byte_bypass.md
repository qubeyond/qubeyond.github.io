...
Раз зарос работает корретно, значит путь до нашего файла должен заканчиваться на `.jpg`. Чтобы в командой строке корректно обработалось имя, поставим нулевой символ `%00` перед расширением:
